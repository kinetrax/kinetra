# KinetraX Bilingual Glossary

Extended EN/RU term reference for consistent content across the site.

## Core Platform Terms

| English | Russian | Context / Notes |
|---------|---------|-----------------|
| KinetraX | KinetraX | Never transliterate to Cyrillic |
| KNT token | Токен KNT | Token ticker stays Latin |
| Trainer | Тренер | Professional who leads sessions |
| Athlete | Спортсмен | User who books sessions |
| Participant | Участник | Generic attendee |
| Workout | Тренировка | Single exercise session |
| Event | Мероприятие | Broader: can include competitions |
| Session | Сессия / Занятие | "Занятие" preferred for sport context |
| Booking | Бронирование | |
| Verification | Верификация | Trainer credential check |
| Rating / Review | Рейтинг / Отзыв | |

## Blockchain & Crypto Terms

| English | Russian | Context / Notes |
|---------|---------|-----------------|
| Blockchain | Блокчейн | Loan word, no translation |
| TON (The Open Network) | TON (The Open Network) | Keep English acronym |
| Toncoin | Toncoin | Native TON currency, keep English |
| Smart contract | Смарт-контракт | Hyphenated in Russian |
| Token | Токен | Loan word |
| Wallet | Кошелёк | |
| Escrow | Эскроу | Loan word |
| Transaction | Транзакция | |
| Gas fee | Комиссия сети | Don't use "газ" |
| Decentralized | Децентрализованный | |
| Jetton | Jetton | TON token standard, keep English |
| DApp | DApp | Keep English |
| KNT payment discounts | Скидки при оплате KNT | Preferred KNT utility wording |
| Liquidity | Ликвидность | |
| Vesting | Вестинг | Loan word |

## UI & Navigation Terms

| English | Russian | Context / Notes |
|---------|---------|-----------------|
| Home | Главная | |
| Benefits | Преимущества | |
| How It Works | Как это работает | |
| Events | События | Nav context |
| FAQ | FAQ | Keep English acronym |
| Whitepaper | Whitepaper | Keep English |
| Privacy Policy | Политика конфиденциальности | |
| Terms of Use | Условия использования | |
| Learn More | Узнать больше | |
| Get Started | Начать | |
| Join | Присоединиться | |
| Sign Up | Зарегистрироваться | |
| Log In | Войти | |
| Connect Wallet | Подключить кошелёк | |

## Action / CTA Terms

| English | Russian | Context / Notes |
|---------|---------|-----------------|
| Book a session | Забронировать занятие | |
| Create event | Создать мероприятие | |
| View details | Подробнее | |
| Subscribe | Подписаться | |
| Download | Скачать | |
| Share | Поделиться | |
| Claim tokens | Получить токены | Token distribution context |
| Use KNT for discounts | Использовать KNT для скидок | Preferred over staking phrasing |
